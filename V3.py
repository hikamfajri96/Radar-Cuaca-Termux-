...
parser.add_argument("--no-banner", action="store_true", help="Skip figlet banner on start")
...
# ---------- FIGLET BANNER (try runtime figlet, fallback to static) ----------
def print_banner():
    """
    Cetak banner "Radar Cuaca Ojol".
    - Coba runtime figlet (font 'standard' default). Ubah FONT var jika mau.
    - Hormati args.no_banner dan hanya tampil saat stdout adalah TTY.
    """
    try:
        if getattr(args, "no_banner", False):
            return
    except Exception:
        # args mungkin belum terdefinisi jika dipanggil saat import; skip saja
        return

    if not is_tty():
        return

    # Pilih font figlet yang kamu suka: 'standard', 'big', 'slant', dsb.
    FONT = "standard"
    ...
    try:
        ...
        if _which("figlet"):
            try:
                out = _sub.check_output(["figlet", "-f", FONT, "-w", "80", "Radar Cuaca Ojol"], stderr=_sub.DEVNULL)
                ...
                # jika figlet error, jatuhkan ke fallback
                pass
    except Exception:
        pass

    # Fallback statis (telah diverifikasi menulis "Radar Cuaca Ojol")
    STATIC_BANNER = r"""
 ____            _                ____                _       
|  _ \ ___  __ _| | _____  ___   / ___|___  _ __ ___ | |__  
| |_) / _ \/ _` | |/ / _ \/ __| | |   / _ \| '_ ` _ \| '_ \ 
|  _ <  __/ (_| |   <  __/\__ \ | |__| (_) | | | | | | |_) |
|_| \_\___|\__,_|_|\_\___||___/  \____\___/|_| |_| |_|_.__/ 
                                                          
 ____                _           ____        _             
/ ___|___  _ __  ___| |_ _   _  / ___|  ___ | |_ ___  _ __ 
| |   / _ \| '_ \/ __| __| | | | \___ \ / _ \| __/ _ \| '__|
| |__| (_) | | | \__ \ |_| |_| |  ___) | (_) | || (_) | |   
 \____\___/|_| |_|___/\__|\__, | |____/ \___/ \__\___/|_|   
                          |___/                            
"""
    try:
        if _USE_COLORS:
            print(f"{BOLD}{CYAN}{STATIC_BANNER}{RESET}")
        else:
            print(STATIC_BANNER)
    except Exception:
        pass
